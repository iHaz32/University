...
int main() {
    ...
    for (int i = 0; i < N; i++) {
        A[i] = (long double *)malloc(3 * sizeof(long double));
    }

    AT = (long double **)malloc(3 * sizeof(long double *));
    for (int i = 0; i < 3; i++) {
    ...
    ATb = (long double *)malloc(3 * sizeof(long double));
    ATb2 = (long double *)malloc(3 * sizeof(long double));

    ATA = (long double **)malloc(3 * sizeof(long double *));
    P = (long double **)malloc(3 * sizeof(long double *));
    L = (long double **)malloc(3 * sizeof(long double *));
    U = (long double **)malloc(3 * sizeof(long double *));
    for (int i = 0; i < 3; i++) {
        ATA[i] = (long double *)malloc(3 * sizeof(long double));
        P[i] = (long double *)malloc(3 * sizeof(long double));
        L[i] = (long double *)malloc(3 * sizeof(long double));
        U[i] = (long double *)malloc(3 * sizeof(long double));
    }
    ...
    //code
    transposeMatrix(A, AT, N, 3);
    multiplyMatrices(AT, A, ATA, 3, N, N, 3);

    multiplyMV(AT, b, ATb, 3, N, N);
    x = solve(P, ATA, L, U, ATb, y, x, 3);

    multiplyMV(AT, b2, ATb2, 3, N, N);
    x2 = solve(P, ATA, L, U, ATb2, y2, x2, 3);
    
    //outputs
    printf("XPR: p2(x) = %Lfx*x + %Lfx + %Lf\n", x[2], x[1], x[0]);
    printf("ETH: p2(x) = %Lfx*x + %Lfx + %Lf\n\n", x2[2], x2[1], x2[0]);
