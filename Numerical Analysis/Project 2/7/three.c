...
void approx(long double *a, long double x, const char* date, char* coinName) {
    long double approx = a[0] + a[1]*x + a[2]*x*x + a[3]*x*x*x;
...
int main() {
    ...
    for (int i = 0; i < N; i++) {
        A[i] = (long double *)malloc(4 * sizeof(long double));
    }

    AT = (long double **)malloc(4 * sizeof(long double *));
    for (int i = 0; i < 4; i++) {
    ...
    ATb = (long double *)malloc(4 * sizeof(long double));
    ATb2 = (long double *)malloc(4 * sizeof(long double));

    ATA = (long double **)malloc(4 * sizeof(long double *));
    P = (long double **)malloc(4 * sizeof(long double *));
    L = (long double **)malloc(4 * sizeof(long double *));
    U = (long double **)malloc(4 * sizeof(long double *));
    for (int i = 0; i < 4; i++) {
        ATA[i] = (long double *)malloc(4 * sizeof(long double));
        P[i] = (long double *)malloc(4 * sizeof(long double));
        L[i] = (long double *)malloc(4 * sizeof(long double));
        U[i] = (long double *)malloc(4 * sizeof(long double));
    }
    ...
    //code
    transposeMatrix(A, AT, N, 4);
    multiplyMatrices(AT, A, ATA, 4, N, N, 4);

    multiplyMV(AT, b, ATb, 4, N, N);
    x = solve(P, ATA, L, U, ATb, y, x, 4);

    multiplyMV(AT, b2, ATb2, 4, N, N);
    x2 = solve(P, ATA, L, U, ATb2, y2, x2, 4);
    
    //outputs
    printf("XPR: p3(x) = %Lfx*x*x + %Lfx*x + %Lfx + %Lf\n", x[3], x[2], x[1], x[0]);
    printf("ETH: p3(x) = %Lfx*x*x + %Lfx*x + %Lfx + %Lf\n\n", x2[3], x2[2], x2[1], x2[0]);
